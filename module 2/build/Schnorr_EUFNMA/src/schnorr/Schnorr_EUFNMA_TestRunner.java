...
import schnorr.reductions.A_Schnorr_EUFNMA_Reduction;
import schnorr.reductions.Schnorr_EUFNMA_Reduction;

public class Schnorr_EUFNMA_TestRunner extends StudentTestRunner {
    private class TestingTuple {
        Class<? extends I_DLog_Challenger<IGroupElement>> challengerClass;
        Class<? extends Schnorr_EUFNMA_Adversary> advClass;
        int numMessages;
        int n;
        int t;

        public TestingTuple(Class<? extends I_DLog_Challenger<IGroupElement>> challengerClass,
                Class<? extends Schnorr_EUFNMA_Adversary> advClass, int numMessages, int n, int t) {
            this.challengerClass = challengerClass;
        ...
        public String toString() {
            String ret = "";
            ret = "Challenger: " + challengerClass.getName()
                    + " Adversary: " + advClass.getName()
                    + " numMess: " + numMessages
                    + " t/n: " + t + " / " + n;
            return ret;
        }
    ...
    protected Class<? extends A_Schnorr_EUFNMA_Reduction> reductionClass;
    protected int groupBitLength;
    private final SecureRandom RNG = new SecureRandom();
    protected List<TestingTuple> test_cases;

    public Schnorr_EUFNMA_TestRunner(
            int groupBitLength, Class<? extends A_Schnorr_EUFNMA_Reduction> reductionClass) {
        setup(reductionClass, groupBitLength);
    }

    private void setup(Class<? extends A_Schnorr_EUFNMA_Reduction> reductionClass, int groupBitLength) {
        this.reductionClass = reductionClass;
        this.groupBitLength = groupBitLength;
        this.test_cases = new ArrayList<TestingTuple>();

        int n = 10;
        int t = 10;
        test_cases.add(new TestingTuple(DLog_Challenger.class, Schnorr_EUFNMA_Adversary.class, 1, n, t));

        n = 132;
        t = 1;
        test_cases.add(new TestingTuple(DLog_Challenger.class, Schnorr_EUFNMA_Adversary.class, 10, n, t));
        test_cases
                .add(new TestingTuple(DLog_Challenger.class, Schnorr_EUFNMA_hashConst_Adversary.class, 10, n, t));
    }
    ...
    private int runTest(TestingTuple test) throws Exception {
        ...
        for (int i = 0; i < test.n; i++) {
            ...
            var challenger = test.challengerClass.getDeclaredConstructor(IGroupElement.class).newInstance(generator);
            ...
            var reduction = reductionClass
                    .getDeclaredConstructor(I_Schnorr_EUFNMA_Adversary.class)
    ...
    public static void main(String[] args) {
        var testrunner = new Schnorr_EUFNMA_TestRunner(
                128, Schnorr_EUFNMA_Reduction.class);
        testrunner.execute();
    }
